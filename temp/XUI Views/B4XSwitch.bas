...
Sub Class_Globals
	...
	Private OnColor, OffColor As BCBrush
	Private bc As BitmapCreator
	Private Width As Int = 55dip
	Private Height As Int = 31dip
	Private ThumbColor As BCBrush
...
Public Sub DesignerCreateView (Base As Object, Lbl As Label, Props As Map)
	...
	#End If
	OnColor = bc.CreateBrushFromColor(xui.PaintOrColorToColor(Props.Get("OnColor")))
	OffColor = bc.CreateBrushFromColor(xui.PaintOrColorToColor(Props.Get("OffColor")))
	ThumbColor = bc.CreateBrushFromColor(xui.PaintOrColorToColor(Props.Get("ThumbColor")))
